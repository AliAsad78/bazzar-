...
// ── showNotif — defined here so the module can always call it ─
// Falls back to window.showNotif if the page defined its own version,
// otherwise creates and injects a minimal notif element on the fly.
function showNotif(msg) {
  if (typeof window.showNotif === "function") {
    window.showNotif(msg);
    return;
  }
  // Fallback: create a simple toast if the page doesn't have one
...
window.doLogin = async function() {
  ...
  try {
    ...
    window.currentProfile = snap.exists() ? snap.data() : {};
    window.closeAuthModal();
    ...
    if (window.currentProfile.role === "seller") {
      // seller stays on current page — they can navigate via header
    }
  } catch (err) {
    errEl.textContent = friendlyError(err.code);
