...
onAuthStateChanged(auth, async (user) => {
  ...
  if (window.location.pathname.includes("bazaar-seller")) {
    ...
    if (!gate) {
      // Gate HTML not present — fallback silent guard
      if (!user) {
    ...
    if (!user) {
      ...
      // Wire buttons to open auth modal on home page (redirect with action param)
      document.getElementById("deniedPrimaryBtn").onclick = () => {
        window.location.href = "index.html?auth=register";
      };
      document.getElementById("deniedSecondaryBtn").onclick = () => {
        window.location.href = "index.html?auth=login";
      };

    } else if (window.currentProfile && window.currentProfile.role !== "seller") {
      // Signed in but not a seller — explain and offer upgrade
      gate.classList.add("hiding");
      setTimeout(() => { gate.classList.add("hidden"); }, 350);
      denied.classList.add("show");

      const icon    = document.getElementById("deniedIcon");
      const title   = document.getElementById("deniedTitle");
      const sub     = document.getElementById("deniedSub");
      const actions = document.getElementById("deniedActions");
      const name    = window.currentProfile.name
                        ? window.currentProfile.name.split(" ")[0]
                        : "there";

      icon.textContent  = "🛍️";
      icon.className    = "denied-icon wrong-role";
      title.textContent = `Hi ${name} — this area is for sellers`;
      sub.innerHTML     = `Your account is set up as a <strong>buyer</strong>. To access the Seller Center, create a separate seller account or upgrade your existing account to seller.`;
      actions.innerHTML = `
        <button class="denied-btn-primary" onclick="window.location.href='index.html?auth=register&role=seller'">
          Create Seller Account — Free
        </button>
        <div class="denied-divider"><span>or</span></div>
        <button class="denied-btn-secondary" onclick="window.location.href='profile.html'">
          Go to My Profile
        </button>
        <a href="index.html" class="denied-btn-secondary" style="margin-top:4px">
          ← Back to Marketplace
        </a>`;

    } else if (window.currentProfile && window.currentProfile.role === "seller") {
      // ✅ Authorised seller — lift the gate
      document.body.classList.add("auth-ready");
      gate.classList.add("hiding");
      setTimeout(() => { gate.classList.add("hidden"); }, 350);

    } else {
      // Profile fetch failed (Firestore error) but user is signed in
      // Be lenient — lift gate, let page load, they can try again
      document.body.classList.add("auth-ready");
      gate.classList.add("hiding");
      setTimeout(() => { gate.classList.add("hidden"); }, 350);
    }
...
window.doRegister = async function() {
  ...
  btn.textContent = "Creating account…"; btn.classList.add("loading"); btn.disabled = true;
  ...
  try {
    ...
    btn.textContent = "Create Account"; btn.classList.remove("loading"); btn.disabled = false;
  }
};
...
window.doUpdateProfile = async function() {
  ...
  const shop  = document.getElementById("prof-shop")?.value.trim()  || "";
  const isSeller = window.currentProfile?.role === "seller";

  let valid = true;
  if (name.length < 2) { markErr("prof-name",  "err-prof-name",  "Please enter your full name."); valid = false; }
  else                  markOk("prof-name",  "err-prof-name");

  const phoneOk = /^0[0-9]{3}[-\s]?[0-9]{7}$/.test(phone.replace(/\s/g,""));
  if (!phoneOk) { markErr("prof-phone", "err-prof-phone", "Enter a valid number (e.g. 0300-1234567)."); valid = false; }
  else           markOk("prof-phone", "err-prof-phone");

  if (isSeller && shop.length < 2) { markErr("prof-shop", "err-prof-shop", "Shop name is required."); valid = false; }
  else if (isSeller)                 markOk("prof-shop", "err-prof-shop");

  if (!valid) {
    if (errEl) errEl.textContent = "Please fix the errors above before saving.";
    // Scroll to first invalid field
    const first = document.querySelector(".form-input.invalid, .form-textarea.invalid");
  ...
  if (isSeller) updates.shopName = shop;
